...
int64 nChainStartTime = 1452502800;
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // TMCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
     ...
     if(nHeight == 1)
     {
       nSubsidy = 3000000 * COIN;	// 3 million coins, that all pow coins
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in TMCoin
    bool fStrictPayToScriptHash = true; // Always active in TMCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "TMCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "TMCoin";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1452502800;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1631945;
        ...
        assert(block.hashMerkleRoot == uint256("0xb4d2fd3ec70e9200306deb84959ed801603b501b864cf248c43ae10834f334cd"));
